		    ...
		    int buflen)
{
     ...
     parse->path = full_addr + 1;

     if(strlen(full_addr)==1) parse->path = "index.html";
     else parse->path = full_addr + 1;						// parse->path added
     ...
     /* Parse protocol would've contained "http" in version 1.0 so not needed now
